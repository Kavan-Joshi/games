...
def upload_amazon_dataset(file):
    if file is None:
    ...
    if "ticket_id" not in headers or "issue_type" not in headers:
        return gr.update(value='❌ Invalid format. Expected columns: ticket_id, issue_type, description, priority, etc.'), "", ""

    total = len(rows)
    issue_types = {}
    for r in rows:
        it = str(r[headers.index("issue_type")] or "Unknown") if headers.index("issue_type") < len(r) else "Unknown"
        issue_types[it] = issue_types.get(it, 0) + 1

    type_rows = ""
    for it, count in sorted(issue_types.items()):
    ...
    for i, r in enumerate(rows[:5]):
        tid = str(r[headers.index("ticket_id")] or f"ROW-{i+1}")
        it = str(r[headers.index("issue_type")] or "N/A")
        sub = str(r[headers.index("sub_issue")] or "N/A") if "sub_issue" in headers else "N/A"
        preview_rows += f'<tr><td style="padding:3px 6px; color:#e2e8f0; font-size:11px;">{tid}</td><td style="padding:3px 6px; color:#a78bfa; font-size:11px;">{it}</td><td style="padding:3px 6px; color:#8888aa; font-size:11px;">{sub}</td></tr>'
    ...
    summary = (
        ...
        f'<div style="font-size:14px; color:#e2e8f0; margin-bottom:4px;">📂 <strong>{file.name}</strong></div>'
        f'<div style="color:#8888aa; font-size:12px;">{total} tickets | {len(issue_types)} issue types | Columns: {len(headers)}</div></div>'
        ...
        f'<tr style="color:#8888aa; border-bottom:1px solid #4a4a6a;"><th style="text-align:left; padding:4px 8px;">Issue Type</th><th style="text-align:left; padding:4px 8px;">Count</th></tr>'
        ...
        f'<tr style="color:#8888aa; border-bottom:1px solid #4a4a6a;"><th style="text-align:left; padding:3px 6px;">Ticket ID</th><th style="text-align:left; padding:3px 6px;">Type</th><th style="text-align:left; padding:3px 6px;">Sub-Issue</th></tr>'
...
def load_amazon_ticket(index):
    ...
    tid = str(d.get("ticket_id") or f"ROW-{index+1}")
    issue_type = str(d.get("issue_type") or "General")
    sub_issue = str(d.get("sub_issue") or "")
    desc = str(d.get("description") or "")
    notes = str(d.get("notes") or "")
    customer_id = str(d.get("customer_id") or "C-000")

    classification = ISSUE_TYPE_TO_CLASSIFICATION.get(issue_type, "general")
    priority_raw = str(d.get("priority") or "Medium").strip()
    priority = PRIORITY_MAP.get(priority_raw, "medium")

    negative_words = ["angry", "frustrated", "terrible", "worst", "refund", "complaint", "overcharged", "not received", "stolen", "broken", "damage"]
    sentiment = "negative" if any(w in f"{desc} {notes}".lower() for w in negative_words) else "neutral"
    ...
    ticket = TicketRecord(id=tid, subject=f"[{issue_type}] {sub_issue}", body=desc, customer_id=customer_id, customer_name=customer_id, customer_tier=TIER_MAP.get(notes.lower(), "silver"), customer_tenure_days=180, sentiment=sentiment, previous_ticket_count=2, ground_truth=gt)
    ...
    ticket_html = card("🎫 Ticket", (
        f'<div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-bottom:10px;">'
        f'<div style="padding:8px; background:#12122a; border-radius:8px;"><span style="color:#8888aa; font-size:11px;">ID</span><br><span style="color:#e2e8f0; font-weight:600;">{tid}</span></div>'
        f'<div style="padding:8px; background:#12122a; border-radius:8px;"><span style="color:#8888aa; font-size:11px;">Type</span><br><span style="color:#a78bfa; font-weight:600;">{issue_type} - {sub_issue}</span></div>'
        f'<div style="padding:8px; background:#12122a; border-radius:8px;"><span style="color:#8888aa; font-size:11px;">Sentiment</span><br><span style="font-size:16px;">{sent_emoji} {sentiment.title()}</span></div>'
        f'<div style="padding:8px; background:#12122a; border-radius:8px;"><span style="color:#8888aa; font-size:11px;">Notes</span><br><span style="color:#8888aa; font-size:12px;">{notes}</span></div></div>'
        f'<div style="padding:10px; background:#12122a; border-radius:8px; margin-bottom:6px;"><span style="color:#8888aa; font-size:11px;">Description</span><br><span style="color:#e2e8f0;">{desc}</span></div>'
    ), "#a78bfa")

    worker_html = card("🤖 Generated Worker Response", (
        f'<div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:8px; margin-bottom:10px;">'
        f'<div style="padding:8px; background:#12122a; border-radius:8px;"><span style="color:#8888aa; font-size:11px;">Classification</span><br><span style="color:#a78bfa; font-weight:600;">{worker.classification or "N/A"}</span></div>'
        f'<div style="padding:8px; background:#12122a; border-radius:8px;"><span style="color:#8888aa; font-size:11px;">Priority</span><br><span style="color:{priority_color}; font-weight:600;">{worker.priority or "N/A"}</span></div>'
        f'<div style="padding:8px; background:#12122a; border-radius:8px;"><span style="color:#8888aa; font-size:11px;">Department</span><br><span style="color:#38bdf8; font-weight:600;">{worker.department or "N/A"}</span></div></div>'
    ...
    info_text = f"Ticket {tid} ({issue_type}/{sub_issue}) loaded.\nInjected errors: {errors_str}"

    notify(f"Amazon ticket loaded: {tid}", "info")
    return gr.update(), ticket_html, worker_html, info_text, gr.update(minimum=0, maximum=len(rows)-1, value=index)


def submit_amazon_inspection(flagged, flagged_fields, issues_json, corrections_json, confidence):
    if "_amazon_ticket" not in state:
        return gr.update(visible=True, value="❌ Load an Amazon ticket first!"), ""
...
with gr.Blocks(title="FleetAI Dashboard") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("Dataset"):
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ticket_slider = gr.Slider(0, 29, value=0, step=1, label="Ticket Index", visible=False)
                    load_btn = gr.Button("➡️ Load This Ticket", visible=False)
    ...
    upload_btn.click(fn=upload_amazon_dataset, inputs=[upload], outputs=[upload_summary, upload, ticket_slider])
    ...
    load_btn.click(fn=load_amazon_ticket, inputs=[ticket_slider], outputs=[ds_status_display, ticket_display2, worker_display2, upload_info, ticket_slider])
    ...
    ds_submit_btn.click(fn=submit_amazon_inspection, inputs=[ds_flagged_check, ds_flagged_fields, ds_issues, ds_corrections, ds_confidence], outputs=[ds_results_display, ds_action_display])
